...
const CAMPOS_PEDIDO = {
  ...
  estado: "estado",
  modoAtencion: "modo_atencion",
  items: "items",
  fotoOriginal: "foto_original",
  creadoEn: "creado_en",
...
router.get("/", async (req, res) => {
  try {
    ...
    // dejamos fuera de la lista para no cargar cada foto en cada refresco.
    // "tiene_foto" es una columna calculada en la base de datos.
    let query = supabase
      .from("pedidos")
      .select(
        "id, cliente, vendedor_id, vendedor_nombre, estado, modo_atencion, " +
        "items, tiene_foto, creado_en, visto_por_vendedor, almacenero_id, " +
