...
router.post("/", async (req, res) => {
  try {
    const { cliente, vendedorId, vendedorNombre, items, foto } = req.body;
...
router.get("/", async (req, res) => {
  try {
    ...
    // La lista no necesita la foto de cada pedido (pesaría mucho de más);
    // solo se envía cuando se pide un pedido puntual.
    const lista = snap.docs.map(d => {
      const { foto, ...resto } = d.data();
      return resto;
    });
